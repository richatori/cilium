...
func (h *nodeNeighRemover) Handle(params DeleteClusterNodesNeighParams) middleware.Responder {
	...
	if params.Addresses.IPV4 == nil && params.Addresses.IPV6 == nil {
		return NewDeleteClusterNodesNeighNotFound()
	}

	node := nodeTypes.Node{}
	node.IPAddresses = make([]nodeTypes.Address, 0, 2)
	if params.Addresses.IPV4 != nil {
		node.IPAddresses = append(node.IPAddresses, nodeTypes.Address{
			Type: addressing.NodeInternalIP,
			IP:   net.ParseIP(params.Addresses.IPV4.IP),
		})
	}
	if params.Addresses.IPV6 != nil {
		node.IPAddresses = append(node.IPAddresses, nodeTypes.Address{
			Type: addressing.NodeInternalIP,
			IP:   net.ParseIP(params.Addresses.IPV6.IP),
		})
	}
	h.d.Datapath().Node().NodeNeighRemove(node)
